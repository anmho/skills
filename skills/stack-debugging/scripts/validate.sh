...
[[ "${name}" == "stack-debugging" ]] || fail "SKILL.md name must be stack-debugging (got: ${name:-empty})"

for required in "proof ladder" "/Users/andrewho/repos/projects/auth" "/Users/andrewho/repos/projects/create-svc" "/Users/andrewho/repos/projects/terraform" "/Users/andrewho/repos/projects/agent" "authctl doctor --json" "service doctor" "bluebubbles server info --json"; do
  grep -q "${required}" "${SKILL_MD}" || fail "SKILL.md missing required text: ${required}"
done

skill_name="$(jq -r '.skill_name' "${EVALS_JSON}")"
[[ "${skill_name}" == "stack-debugging" ]] || fail "evals.json skill_name must be stack-debugging"
...
echo "stack-debugging skill validation OK (${count} evals)"
