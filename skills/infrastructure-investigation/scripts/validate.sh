...
[[ "${name}" == "infrastructure-investigation" ]] || fail "SKILL.md name must be infrastructure-investigation (got: ${name:-empty})"

for required in "Grafana" "Loki" "Prometheus" "Cloud Run" "Cloudflare" "fault domain"; do
  rg -n "${required}" "${SKILL_MD}" >/dev/null || fail "SKILL.md missing required text: ${required}"
done

skill_name="$(jq -r '.skill_name' "${EVALS_JSON}")"
[[ "${skill_name}" == "infrastructure-investigation" ]] || fail "evals.json skill_name must be infrastructure-investigation"
...
echo "infrastructure-investigation skill validation OK (${count} evals)"
